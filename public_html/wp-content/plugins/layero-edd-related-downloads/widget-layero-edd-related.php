...
class layero_edd_related_downloads_widget extends WP_Widget {
	public function __construct() {
		parent::__construct(
	 		'layero_edd_related_downloads_widget',
			esc_html__('EDD Related Downloads by Layro', 'layero-edd-related-downloads'),
			array( 'description' => esc_html__( 'Display related downloads.', 'layero-edd-related-downloads' ), )
		);
	}
	/**
	...
	public function widget( $args, $instance ) {
		...
		$title = apply_filters( 'widget_title', empty( $instance['title'] ) ? esc_html__( 'You May Also Like', 'layero-edd-related-downloads' ) : $instance['title'], $instance, $this->id_base );
		...
		if( is_single() && ( 'download' == get_post_type() ) ) {
			...
			if ( ! empty($custom_taxterms) ) {
				...
				if( $eddrdw_query->have_posts() ) {
					...
					if ( $title ) {
						echo '<div class="sidebar-title">'. $title . '</div>';
					} ?>
					<div id="layero-edd-related-downloads-widget">
					<ul>
					<?php 
	...
	public function form( $instance ) {
		$defaults = array( 
				'title' => esc_html__('You May Also Like','layero-edd-related-downloads'),
				'number' => 3,
				'taxcat' => false);
 		$instance = wp_parse_args( (array) $instance, $defaults );
    	?>
		<p><label for="<?php echo $this->get_field_id( 'title' ); ?>"><?php esc_html_e( 'Title:', 'layero-edd-related-downloads' ); ?></label><input class="widefat" id="<?php echo $this->get_field_id( 'title' ); ?>" name="<?php echo $this->get_field_name( 'title' ); ?>" type="text" value="<?php echo $instance['title']; ?>" /></p>
		<p><label for="<?php echo $this->get_field_id( 'number' ); ?>"><?php esc_html_e( 'How many related downloads to show: ', 'layero-edd-related-downloads' ); ?></label><input type="text" class="widefat" id="<?php echo $this->get_field_id( 'number' ); ?>" name="<?php echo $this->get_field_name( 'number' ); ?>" value="<?php echo $instance['number']; ?>" /></p>
		<p><input id="<?php echo $this->get_field_id( 'taxcat' ); ?>" name="<?php echo $this->get_field_name( 'taxcat' ); ?>" type="checkbox" class="checkbox" <?php checked( $instance['taxcat'], 'on' ); ?> /><label for="<?php echo $this->get_field_id( 'taxcat' ); ?>"><?php _e( ' Filter By Category Instead of Tag', 'layero-edd-related-downloads' ); ?></label></p>
		<?php 
	}
}
?>